...
int main() {
    ...
    std::vector<std::string> record1 = { "1", "Alice", "30", "Woman", "1" };
    std::vector<std::string> record2 = { "2", "Bob", "25", "Man", "2" };
    ...
    std::cout << std::endl;
    std::cout << "添加外键：";
    tableManager.alter_addForeignKey(dbName, table1, "DeptID", "Departments", "ID", Table::ForeignKeyAction::NOACTION, Table::ForeignKeyAction::CASCADE);
    tableManager.describeTable(table1);
...
//    tableManager.orderByRecord(dbName, table1, record4, { "ASC" }, fieldNames);
    ...
    std::cout << std::endl;
    std::cout << "更新后读取全表：";
    std::vector<std::string> updateColumn = { "DeptID" };
